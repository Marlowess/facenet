    ...
    with tf.variable_scope(scope, 'resnet_v1', [inputs], reuse=reuse) as sc:
        ...
        with slim.arg_scope([slim.conv2d, bottleneck,
            ...
            if is_training is not None else NoOpScope()):
                ...
                net = stack_blocks_dense(net, blocks, output_stride,
                                         store_non_strided_activations)
                net = tf.layers.batch_normalization(net, training=is_training, name="last_batch_norm_1")
                net = tf.nn.dropout(net, keep_prob=0.4, name='E_Dropout')
                net = tf.compat.v1.layers.flatten(net)
                net = tf.compat.v1.layers.dense(net, units=num_classes, name="last_dense_layer")
                net = tf.layers.batch_normalization(net, training=is_training, name="last_batch_norm_2")
                return net


resnet_v1.default_image_size = 128
